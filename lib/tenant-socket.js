            ...
            socket.on('player-update', (data) => {
                ...
                try {
                    ...
                    if (data?.adminInvisible !== undefined && socket.data.isAdmin) {
                        const nextInvisible = !!data.adminInvisible;
                        if (player.adminInvisible !== nextInvisible) {
                            player.adminInvisible = nextInvisible;
                            state.players.set(socket.id, player);
                            emitPlayersUpdate(io, currentRoom, state);
                        }
                    }
