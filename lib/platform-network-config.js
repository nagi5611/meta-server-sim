// lib/platform-network-config.js — 系列サーバー / ポートマップ / ナビリンク（.env + data/platform 統合）
 ...
 */
function emptyNetworkConfigFile() {
    return {
        portalLinks: [],
        servers: [],
        tenantAccessUrls: [],
...
function normalizeNetworkConfigFile(raw) {
    const base = emptyNetworkConfigFile();
    ...
    return {
        ...
        servers,
        tenantAccessUrls,
        proxyServiceDomain: String(raw.proxyServiceDomain ?? '').trim().toLowerCase(),
        useReverseProxy: !!raw.useReverseProxy,
        trustProxy: !!raw.trustProxy,
        requireSecureHttp: !!raw.requireSecureHttp,
    };
}
...
function buildNetworkConfigFromEnv() {
    const envPortal = parseMetaversePortalLinks(process.env.METAVERSE_PORTAL_LINKS);
    const { map } = parseProxyDomainPortMapSafe(process.env.PROXY_DOMAIN_PORT_MAP);
    const servers = [...map.entries()].map(([host, port]) => ({
        label: host,
        host,
        port,
        secure: isTruthyConfig(process.env.REQUIRE_SECURE_HTTP),
    }));
    ...
    return {
        ...
        servers,
        tenantAccessUrls: [],
        proxyServiceDomain: String(process.env.PROXY_SERVICE_DOMAIN || '').trim().toLowerCase(),
        useReverseProxy: isTruthyConfig(process.env.USE_REVERSE_PROXY),
        trustProxy: isTruthyConfig(process.env.TRUST_PROXY),
        requireSecureHttp: isTruthyConfig(process.env.REQUIRE_SECURE_HTTP),
    };
}
    ...
    if (!cachedFileConfig) {
        ...
        cachedFileConfig = fromDisk
            ? {
                  ...
                  proxyServiceDomain:
                      fromDisk.proxyServiceDomain || fromEnv.proxyServiceDomain,
                  useReverseProxy: fromDisk.useReverseProxy,
                  trustProxy: fromDisk.trustProxy,
                  requireSecureHttp: fromDisk.requireSecureHttp,
              }
            : fromEnv;
    }
...
export function getProxyDomainPortMap() {
    ...
    const env = parseProxyDomainPortMapSafe(process.env.PROXY_DOMAIN_PORT_MAP);
...
function buildSocketCorsOrigins(cfg) {
    ...
    for (const raw of String(process.env.SOCKET_CORS_ORIGINS || '').split(/[\r\n,]+/)) {
        const o = raw.trim();
        if (o) origins.add(o);
    }
    for (const s of cfg.servers) {
...
export function resolveServerBootConfig() {
    ...
    if (useReverseProxy && proxyServiceDomain) {
        if (proxyDomainPortMap.size === 0) {
            ...
            if (
                ...
                process.env.PORT !== '' &&
                parseInt(process.env.PORT, 10) !== mappedPort
            ) {
                warnings.push(
                    `PORT (${process.env.PORT}) と ${proxyServiceDomain} のマップ (${mappedPort}) が不一致。マップのポートを使用します。`
                );
            }
    ...
    const envMapErrors = parseProxyDomainPortMapSafe(process.env.PROXY_DOMAIN_PORT_MAP).errors;
...
export function validateNetworkConfigPayload(payload) {
    ...
    if (normalized.useReverseProxy && normalized.proxyServiceDomain) {
        if (!hosts.has(normalized.proxyServiceDomain)) {
            errors.push('このサーバーのドメインが系列サーバー一覧にありません');
        }
    }
...
export function saveNetworkConfigToDisk(config) {
    ...
    const payload = {
        ...
        proxyServiceDomain: config.proxyServiceDomain,
        useReverseProxy: config.useReverseProxy,
        trustProxy: config.trustProxy,
        requireSecureHttp: config.requireSecureHttp,
    };
...
export function getNetworkConfigForAdmin() {
    ...
    return {
        ...
        needsRestartForPort:
            ...
            process.env.PORT !== undefined &&
            String(boot.port) !== String(process.env.PORT),
    };
}
