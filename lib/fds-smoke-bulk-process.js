...
/**
 * バルク子 fork 前に親子で同一の SOCKET_AUTH_SECRET を使う（未設定時は親 env に生成して fork へ伝播）
 */
export function ensureSharedSocketAuthSecretBeforeFork() {
    if (String(process.env.SOCKET_AUTH_SECRET || '').trim()) {
        return;
    }
    process.env.SOCKET_AUTH_SECRET = crypto.randomBytes(32).toString('hex');
    console.warn(
        '[fds-smoke-bulk] SOCKET_AUTH_SECRET unset; generated ephemeral secret for main + bulk child. Set SOCKET_AUTH_SECRET in .env for stable tokens across restarts.'
    );
}
